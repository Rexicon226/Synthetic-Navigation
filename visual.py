...
def main():
    sg.theme('DarkAmber')
    column_to_be_centered = [[sg.InputText(size=5, key='rows'), sg.Text('Rows')],
                             [sg.InputText(size=5, key='col'), sg.Text('Columns')],
                             [sg.InputText(size=5, key='oct'), sg.Text('Octaves')],
                             [sg.InputText(size=5, key='noise'), sg.Text('Noise Level')],
                             [sg.InputText(size=7, key='seed'), sg.Text('Seed')],
                             [sg.Checkbox(text="Show Progress Bar", key='prog', checkbox_color='White')], ]
    layout = [[sg.Column(column_to_be_centered, vertical_alignment='center', justification='left', k='-C-')],
              [sg.Output(size=(60, 7))],
    ...
    while True:  # Event Loop
        ...
        if event == 'Run':
            runCommand(cmd='py -c "import blobcheck; blobcheck.visualize(' + str(values['rows']) + ', '
                           ...
                           + str(values['noise']) + ', ' + str(values['prog']) + ', '
                           + str(values['seed']) + ')"', window=window)
